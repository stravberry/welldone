...
const ServiceDetailPage = () => {
  ...
  } = useStaggeredAnimation<HTMLDivElement>(4, 200);
  const {
    elementRef: benefitsRef,
    visibleItems: visibleBenefits
  } = useStaggeredAnimation<HTMLDivElement>(6, 150);
  const {
    elementRef: statsRef,
  ...
  const stats = [{
    value: 1500,
    label: 'Przeszkolonych operatorów',
    suffix: '+'
  }, {
    value: 96,
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <div ref={heroRef} className="relative bg-gradient-to-br from-orange-600 via-orange-500 to-orange-400 py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          {/* Animated Breadcrumb */}
          <div className="flex items-center space-x-2 text-orange-100 mb-8" style={{
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-block bg-orange-100 text-orange-800 px-4 py-2 rounded-full text-sm font-medium mb-6" style={{
              opacity: heroInView ? 1 : 0,
              transform: heroInView ? 'translateY(0)' : 'translateY(20px)',
              ...
              <p className="text-xl text-orange-100 mb-8 leading-relaxed" style={{
              opacity: heroInView ? 1 : 0,
              transform: heroInView ? 'translateY(0)' : 'translateY(20px)',
              transition: 'all 0.8s cubic-bezier(0.4, 0, 0.2, 1)',
              ...
              transitionDelay: '800ms'
                ...
                <Button size="lg" className="bg-white text-orange-600 hover:bg-orange-50 hover:scale-105 transition-all duration-300 shadow-xl" onClick={() => setShowContactForm(true)}>
            ...
            transitionDelay: '600ms'
              ...
              {stats.slice(0, 4).map((stat, index) => <StatPreviewCard key={index} stat={stat} index={index} isVisible={heroInView} />)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto py-20 px-4 sm:px-6 lg:px-8">
        ...
        <div ref={coursesRef} className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {courses.map((course, index) => <EnhancedCourseCard key={course.id} course={course} index={index} isVisible={visibleCourses.includes(index)} onClick={() => handleCourseClick(course.id)} />)}
        </div>
      </div>
      ...
      <div className="bg-white py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div ref={benefitsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {benefits.map((benefit, index) => <div key={index} className="text-center p-6 rounded-xl bg-orange-50 hover:bg-orange-100 transition-all duration-300 hover:scale-105" style={{
      ...
      <div ref={faqRef} className="max-w-4xl mx-auto py-20 px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all duration-300" style={{
          opacity: faqInView ? 1 : 0,
          transform: faqInView ? 'translateY(0)' : 'translateY(20px)',
          transition: 'all 0.6s cubic-bezier(0.4, 0, 0.2, 1)',
          transitionDelay: `${index * 100}ms`
        }}>
              <button className="w-full p-6 text-left flex justify-between items-center hover:bg-orange-50 transition-colors duration-200" onClick={() => toggleFAQ(index)}>
                <h3 className="text-lg font-semibold text-gray-900 pr-4">{faq.question}</h3>
                {openFAQ === index ? <ChevronUp className="h-5 w-5 text-orange-600 flex-shrink-0" /> : <ChevronDown className="h-5 w-5 text-orange-600 flex-shrink-0" />}
              </button>
      ...
      <Link to="/uslugi" className="fixed bottom-6 left-6 bg-orange-600 text-white p-3 rounded-full shadow-lg hover:bg-orange-700 hover:scale-110 transition-all duration-300 z-50">
...
const EnhancedCourseCard: React.FC<{
  ...
  return <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 hover:scale-105 hover:-translate-y-2 cursor-pointer" style={{
    ...
    transition: 'all 0.6s cubic-bezier(0.4, 0, 0.2, 1)'
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        {/* Badge */}
        {course.badge && <div className="absolute top-4 right-4 bg-white text-orange-600 px-3 py-1 rounded-full text-sm font-medium shadow-lg">
            {course.badge}
          </div>}
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 mb-6">
          {course.features.map((feature, featureIndex) => <div key={featureIndex} className="flex items-center text-sm text-gray-600">
...
const StatPreviewCard: React.FC<{
  ...
  const {
    elementRef,
    count
  } = useCounterAnimation<HTMLDivElement>(stat.value, 1500);
  return <div ref={elementRef} className="text-center p-4 bg-white bg-opacity-20 rounded-xl backdrop-blur-sm" style={{
    opacity: isVisible ? 1 : 0,
