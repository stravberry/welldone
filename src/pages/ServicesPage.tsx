...
const ServicesPage = () => {
  ...
  const stats = [
    { value: 1500, label: 'Zadowolonych uczestników', suffix: '+' },
    { value: 96, label: 'Zdawalność egzaminów', suffix: '%' },
    { value: 10, label: 'Lat doświadczenia', suffix: '+' },
    { value: 200, label: 'Firm partnerskich', suffix: '+' }
  ];

  const features = [
    { icon: <Star className="h-6 w-6" />, text: 'Doświadczeni instruktorzy' },
    { icon: <Award className="h-6 w-6" />, text: 'Certyfikowane kursy' },
    { icon: <Shield className="h-6 w-6" />, text: 'Gwarancja jakości' },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div 
        ref={heroRef}
        className="relative bg-gradient-to-br from-orange-600 via-orange-500 to-orange-400 py-20 px-4 sm:px-6 lg:px-8 overflow-hidden"
      >
          ...
          >
            ...
            >
              {features.map((feature, index) => (
                <div 
                  key={index}
                  className="flex items-center space-x-2 bg-white bg-opacity-20 rounded-full px-4 py-2 text-white"
                >
                  {feature.icon}
...
}> = ({ title, description, icon, link, imageSrc, badge, features, onServiceClick }) => {
